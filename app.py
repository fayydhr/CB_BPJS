...
db_engine = create_engine("mysql+pymysql://root:@localhost:3306/bpjs_db")
...
try:
    loader = PyPDFLoader(os.path.join(app.config["UPLOAD_FOLDER"], "sop_bpjs.pdf"))
    pages = loader.load()
    if pages:
...
def chat():
    try:
        ...
        if not command or not username:
            return jsonify({"error": "Command dan username wajib dikirim"}), 400
        ...
        with db_engine.connect() as conn:
            conn.execute(
                ...
                {"user": username, "bot": response}  # simpan username, bukan isi pertanyaan
            )
...
def get_user_history(username):
    try:
        with db_engine.connect() as conn:
            result = conn.execute(
                text("SELECT user, bot, created_at FROM history WHERE user = :username ORDER BY created_at DESC"),
                {"username": username}
            )
            rows = [dict(row._mapping) for row in result]
        return jsonify(rows)
...
def admin_history():
    ...
    try:
        with db_engine.connect() as conn:
            result = conn.execute(
                text("SELECT user, bot, created_at FROM history ORDER BY created_at DESC")
            )
            history = [
                {
                    "user_id": row.user,  # Menggunakan 'user' sebagai pengganti 'user_id'
                    "role": "unknown",    # Role tidak tersedia di tabel history, jadi diisi 'unknown'
                    "message": row.bot,   # Menggunakan 'bot' sebagai 'message'
